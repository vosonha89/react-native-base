...
export class LanguageService extends AbstractLanguageService<StorageService> {
    ...
    public async initLanguage(): Promise<void> {
        try {
            const stored = await this.storeService.getObject<string>(StorageKey.language);
            const currentLanguage = stored && (stored === LanguageCode.EN || stored === LanguageCode.VI)
                ? stored
                : LanguageCode.VI;
            this.getLanguageText(currentLanguage);
        } catch (err) {
            console.error('LanguageService.initLanguage: failed to read stored language, falling back to default', err);
    ...
    public async setLanguage(value: string): Promise<void> {
        const me = this;
        const normalized = value === LanguageCode.EN ? LanguageCode.EN : LanguageCode.VI;
        try {
            const saved = await me.storeService.saveObject<string>(StorageKey.language, normalized);
            if (!saved) {
                console.warn(`LanguageService.setLanguage: storage save returned false for value=${normalized}`);
            }
        } catch (err) {
            console.error(`LanguageService.setLanguage: failed to persist language=${normalized}`, err);
        } finally {
