...
const NAV_PATH = path.join(ROOT, 'src', 'common', 'components', 'NavigationContainerComponents.tsx');
...
function validateName(name) {
    if (!name) bail('Name is required. Use --name=Profile or enter it interactively.');
    if (!/^[A-Z][a-zA-Z0-9]*$/.test(name)) {
        bail(`Invalid name "${name}". Must be PascalCase, e.g. Profile, UserSettings`);
    }
}
